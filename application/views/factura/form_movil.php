...
function validar(table, opc) {
    ...
    if (opc == 0) {
        ...
        if($('#cantidad').val().length!=0 &&  parseFloat($('#cantidad').val())>0 && $('#descuento').val().length!=0 && 
        $('#pro_descripcion').val().length!=0)
        {
            clona_detalle(table);
            validar_cantidad(0,a1);
        } else {
            validar_cantidad(0);
        }
    } else {
...
function validar_cantidad(opc,obj){
              if(opc==0){
                n=obj;
                if (obj>0) {
                      if($('#cantidad'+n).val().length==0 || parseFloat($('#cantidad'+n).val())<=0){
                        swal("Advertencia!", "Revise que la cantidad sea mayor a 0.!", "warning");
                      ...
                      if($('#pro_precio'+n).val().length==0 ||  parseFloat($('#pro_precio'+n).val())<=0){
                        swal("Advertencia!", "Revise que el precio sea mayor a 0.!", "warning");
                        $('#pro_precio'+n).css({borderColor: "red"});
                        $('#pro_precio'+n).focus();
                      }else{
                        $('#pro_precio'+n).css({borderColor: ""});
                      }
                  }else{

                      swal("Advertencia!", "Revise que la cantidad o precio sean mayor a 0.!", "warning");
                    if($('#cantidad').val().length==0 ||  parseFloat($('#cantidad').val())<=0){
                      $('#cantidad').css({borderColor: "red"});
                      $('#cantidad').focus();
                    }

                    if($('#pro_precio').val().length==0 ||  parseFloat($('#pro_precio').val())<=0){
  ...
  console.log(pro_descripcion.value);
    ...
    if (d == 0) {
        ...
        var fila = "<tr>" +
            ...
            "' name='cantidad" + i + "' lang='" + i + "' onchange='validar_inventario_det(),validar_cantidad(1,this)'  value='" + cantidad
            .value + "' onkeyup='validar_decimal(this)'/>" +
            "</td>" +
            "<td style='width:50px'>" +
            "<input type ='text' size='10' style='text-align:right' id='pro_precio" + i + "' name='pro_precio" + i +
            "' onchange='calculo(),validar_cantidad(1,this)' value='" + pro_precio.value + "' lang='" + i +
...
function load_producto(j) {
    if (m_pag == 1 && $('#pag_forma1').val().length == 0) {
        ...
        $.ajax({
            ...
            url: base_url + "factura/load_producto/"  + inven + "/" + ctr_inv + "/" + fpag + "/" +
                emi_id.value + "/0",
            type: 'POST',
            dataType: 'JSON',
            data: parametros,
