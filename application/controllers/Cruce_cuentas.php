...
class Cruce_cuentas extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_facturas=$this->cruce_cuentas_model->lista_facturas_pendientes($text,$f1,$f2,$estado,$rst_cja->emp_id);
		}else{
			$text= '';
			$f1= date('Y-m-d');
			$f2= date('Y-m-d');
	...
	public function guardar($opc_id){
		...
		if($saldo==0){
			 $data=array('reg_estado'=>'22');
			 $this->reg_factura_model->update($reg_id,$data);
		}
	...
	public function asientos($id){
        ...
        $data = Array(
                        ...
                        'con_concepto_debe'=>$rst_cta->pln_id,
    ...
    public function show_frame($id,$opc_id){
    	...
    	if($permisos->rop_reporte){
    		$data=array(
					...
					'direccion'=>"cruce_cuentas/reporte_cruce/$id",
				);
			$this->load->view('layout/header',$this->menus());
			$this->load->view('layout/menu',$this->menus());
    ...
    public function reporte_cruce($id){
        ...
        $pdf->Cell(20, 5, "ASIENTO", 'TB', 0, 'C');
        $pdf->Cell(25, 5, "CODIGO", 'TB', 0, 'C');
        $pdf->Cell(40, 5, "CUENTA", 'TB', 0, 'C');
        $pdf->Cell(25, 5, "DOCUMENTO", 'TB', 0, 'C');
        $pdf->Cell(30, 5, "CONCEPTO", 'TB', 0, 'C');
        ...
        foreach($cns_cuentas as $rst_cuentas) {
            ...
            if (!empty($rst_cuentas->con_concepto_debe)) {
                array_push($cuentas, $rst_cuentas->con_concepto_debe . '&' . $rst_cuentas->con_id . '&0'.'&'.$rst_cuentas->con_asiento);
            }else{
                array_push($cuentas, $rst_cuentas->con_concepto_debe . '&' . $rst_cuentas->con_id . '&0'.'&'.$rst_cuentas->con_asiento);
            }

            if (!empty($rst_cuentas->con_concepto_haber)) {
                array_push($cuentas, $rst_cuentas->con_concepto_haber . '&' . $rst_cuentas->con_id . '&1'.'&'.$rst_cuentas->con_asiento);
            }else{
                array_push($cuentas, $rst_cuentas->con_concepto_haber . '&' . $rst_cuentas->con_id . '&1'.'&'.$rst_cuentas->con_asiento);
            }
        }
        ...
        while ($n < count($cuentas)) {
            ...
            if ($cta[2] == 0) {
                $rst_v = $this->asiento_model->listar_asientos_debe($cta[3], $cta[0], $cta[1]);
	                $vdebe=$rst_v->con_valor_debe;
	                $vhaber = 0;
            } else {
                $rst_v = $this->asiento_model->listar_asientos_haber($cta[3], $cta[0], $cta[1]);
	                $vdebe = 0;
            ...
            $pdf->Cell(20, 5, $cta[3], 0, 0, 'L');
            $pdf->Cell(25, 5, $rst_cuentas1->pln_codigo, 0, 0, 'L');
            $pdf->Cell(40, 5, substr(strtoupper($rst_cuentas1->pln_descripcion), 0, 24), 0, 0, 'L');
            $pdf->Cell(25, 5, $rst_v->con_documento, 0, 0, 'L');
            $pdf->Cell(30, 5, substr($rst_v->con_concepto, 0, 30), 0, 0, 'L');
