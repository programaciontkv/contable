...
class Reg_nota_credito extends CI_Controller {
	...
	public function guardar($opc_id){
		...
		if($this->form_validation->run()){
		    ...
		    if(!empty($rnc_id)){
		        ...
		        $saldof=0;
		        if(!empty($rst_sal)){
                	$saldof = round($rst_sal->total,$dec) - round($rst_sal->credito,$dec);
            	}

            	if(round($saldof,$dec)>=round($total_valor,$dec)){
		...
		if($this->form_validation->run()){
			...
			if($this->reg_nota_credito_model->update($id,$data)){
		        ...
		        if($conf_as->con_valor==0){
		        	...
		        	//eliminar asientos_pagos
		        	$cns_cxp=$this->reg_nota_credito_model->lista_pagos_ctasxcob($id);
		        	if(!empty($cns_cxp)){
			        	foreach ($cns_cxp as $rst_cp) {
			        		$this->reg_nota_credito_model->delete_asiento_pago($rst_cp->ctp_id);
			        	}
		        	}
	...
	public function show_frame($id,$opc_id){
    	...
    	if($permisos->rop_reporte){
    		$data=array(
					'titulo'=>'Registro Nota de Credito '.ucfirst(strtolower($rst_cja->emp_nombre)),
    ...
    public function asientos_pagos($id){
        ...
        foreach ($cns as $rst) {
            ...
            $cban=$this->plan_cuentas_model->lista_un_plan_cuentas($rst->ctp_banco);
